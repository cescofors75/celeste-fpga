"""Load the isolated Rhythm Lab experiment into volatile SRAM; preserve flash."""
from pathlib import Path
import hashlib,json,re,subprocess,time
import serial
ROOT=Path(__file__).resolve().parent.parent
fs=ROOT/'deliverables/celeste_rhythm_lab.fs'
expected='ca383af1f1235fb32196afdb85d15bb2ac0e6b9636ef53347265e76e5248dc99'
assert hashlib.sha256(fs.read_bytes()).hexdigest()==expected,'Unexpected firmware'
report=(ROOT/'build/rhythm-lab/impl/pnr/celeste_rhythm_lab_tr_content.html').read_text()
...
p.check_returncode()
(ROOT/'build/rhythm-loaded.json').write_text(json.dumps({'sha256':expected,'operation':'SRAM only','programmer_returncode':p.returncode,'programmer_output':p.stdout,'clock_hz':12288000,'listening_validation':'pending','loaded_at':time.strftime('%Y-%m-%dT%H:%M:%S')},indent=2))
