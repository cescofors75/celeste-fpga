...
module celeste_stream_board(
 input wire clk27,clk_audio,reset_button,uart_rx,touch_async,
 ...
 stream_endpoint #(.DEPTH(131072),.DSP(1),.SDRAM(1)) endpoint(clk_audio,ar[2],ep_rx_valid,ep_rx_data,ep_rx_ready,ep_tx_valid,ep_tx_data,ep_tx_ready,pcm_bck,pcm_lrck,pcm_din,monitor_valid,monitor_left,
 encoder_event,encoder_index,encoder_delta,panel_online,controls,mappings,routes,revision,selected,running,touch_pulse,meters,panel_switch,O_sdram_clk,O_sdram_cke,O_sdram_cs_n,O_sdram_ras_n,O_sdram_cas_n,O_sdram_wen_n,O_sdram_addr,O_sdram_ba,O_sdram_dqm,IO_sdram_dq,1'b0,1'b0,1'b0,encoder_press);
 reg uart_fault;
